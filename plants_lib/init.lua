...
local function shrt_pos(p, cord, min)
	if min then
		for n,i in pairs(p) do
			p[n] = math.max(i, cord[n])
		end
		return p
	else
		for n,i in pairs(p) do
			p[n] = math.min(i, cord[n])
		end
		return p
	end
end

local function nodes_in_area(p1, p2, names, data, area, minp, maxp)
	local p1 = shrt_pos(p1, minp, true)
	local p2 = shrt_pos(p2, maxp)
...
local function cnt_nodes_in_area(p1, p2, names, data, area, minp, maxp)
	local p1 = shrt_pos(p1, minp, true)
	local p2 = shrt_pos(p2, maxp)
	names = avoid_groups(names)
	local count = 0
